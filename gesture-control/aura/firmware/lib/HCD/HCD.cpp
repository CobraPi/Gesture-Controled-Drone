...
void HCD::update(unsigned char throttle, unsigned char yaw, unsigned char yawtrim, unsigned char pitch,
                 unsigned char roll, unsigned char pitchtrim, unsigned char rolltrim, unsigned char flyrun) {
...
void HCD::update(unsigned char *payload) {
    ...
    switch (state) {
        case HS_CONNECT:
            ...
            if (c & 0x20)  //ACK
            {
                if (HS_PREVIOUS == HS_UNINITIALIZED) {
                    //Serial.println("ACK");
                    HS_PREVIOUS = state;
                    state = HS_CONNECTED;
                }
